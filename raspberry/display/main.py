...
def init_gpio():
    ...
    # GPIO.add_event_detect(21, GPIO.RISING, callback=display_time, bouncetime=75)
...
def display_time(channel):
    ...
    screen.AddText(sDate, (264-get_text_size(sDate,sMainFontPath,16))/2, 104, size=16, Id='Date', fontPath=sMainFontPath)
    screen.AddText(sTime, (264-get_text_size(sTime,sMainFontPath,64))/2, 40, size=64, Id='Time', fontPath=sMainFontPath)
...
screen = PapirusComposite(False, 0)
screen.AddText(sTitle1, (264-get_text_size(sTitle1,sMainFontPath,32))/2, 40, size=32, Id='Title', fontPath=sMainFontPath)
screen.AddText(sTitle2, (264-get_text_size(sTitle2,sMainFontPath,32))/2, 70, size=32, Id='Title2', fontPath=sMainFontPath)
screen.AddText(sTitle3, (264-get_text_size(sTitle3,sMainFontPath,16))/2, 110, size=16, Id='Title3', fontPath=sMainFontPath)
screen.WriteAll()
time.sleep(2)
...
while True:
    ...
    # Construct display
    screen.AddImg(sBatteryIcon, 236, 0, (32, 32), Id='BattIcon')
    screen.AddText(sCurrentTemp, 60, 40, size=56, Id="Temp", fontPath=sMainFontPath)
    screen.AddText('°C', 64 + get_text_size(sCurrentTemp,sMainFontPath,56), 62, size=32, Id='TempUnit', fontPath=sMainFontPath)
    screen.AddText(sCurrentHumidity, 60, 110, size=56, Id="Humid", fontPath=sMainFontPath)
    screen.AddText('%', 64 + get_text_size(sCurrentHumidity,sMainFontPath,56), 132, size=32, Id='HumidUnit', fontPath=sMainFontPath)
